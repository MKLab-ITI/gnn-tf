    ...
    for node in graph:
        if graph.degree(node) >= 10 and "A" in node:
            ...
            gnn.train(train=gnntf.LinkPrediction(lambda: gnntf.negative_sampling(edges[train], training_graph, samples=1)),
                      valid=gnntf.LinkPrediction(lambda: gnntf.negative_sampling(edges[valid], training_graph, samples=1)),
                      patience=10)

            test_links, labels = gnntf.negative_sampling(edges[test], graph)
