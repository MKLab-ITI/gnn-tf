...
class GRecLayer(Layer):
    ...
    def __forward__(self, gcn, features: tf.Tensor):
        aggregated_features = tf.sparse.sparse_dense_matmul(gcn.get_adjacency(self.graph_dropout, renormalized=False), features)
...
class GRec(GNN):
    ...
    def __init__(self, G: tf.Tensor, features: tf.Tensor, num_classes: int, latent_dims=None, dropout=0, **kwargs):
        super().__init__(G, features, **kwargs)
        ...
        self.add(Dropout(dropout))
        for latent_dim in latent_dims:
            layers.append(self.add(GRecLayer(latent_dim)))
            self.add(Dropout(dropout))
