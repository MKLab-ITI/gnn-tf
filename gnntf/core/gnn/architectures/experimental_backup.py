from gnntf.core.nn import Layer, Dense, Branch, Resume, Tradeoff, Concatenate
...
class APPNP2(GNN):
    ...
    def __init__(self, G: tf.Tensor, features: tf.Tensor, num_classes: int, a: float = 0.1, latent_dims=[64], iterations=40,
        ...
        filter_outcomes = list()
        for avalue in [a, a*2]:
            self.add(Resume(H0))
            for _ in range(iterations):
                self.add(PPRIteration(H0, avalue, graph_dropout=graph_dropout, activation=activation))
            filter_outcomes.append(self.top_layer())
        self.add(Tradeoff(filter_outcomes, weights=[tf.constant(1.), tf.constant(-5.)]))



class APPNPThrough(GNN):
